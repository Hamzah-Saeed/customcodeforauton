...
# from geographic_msgs.msg import GeoPoseArray  # Uncomment if you create a custom subscriber msg type
...
class GpsWpCommander(Node):
    ...
    def __init__(self, wps_file_path):
        ...
        # self.subscription = self.create_subscription(PoseStamped, '/gps_waypoints', self.waypoints_callback, 10)  # Add a subscriber

        # self.received_wps = None  # Storage for incoming waypoints

    # def waypoints_callback(self, msg):
    #     self.received_wps = msg  # Save incoming waypoints
    ...
    def start_wpf(self):
        ...
        # if self.received_wps is not None:
        #     wps = self.received_wps  # Override waypoints if a topic message is received
...
def main():
    ...
    default_yaml_file_path = os.path.join(get_package_share_directory(
        "navigation"), "config", "demo_waypoints.yaml")
    if len(sys.argv) > 1:
